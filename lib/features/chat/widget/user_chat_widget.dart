...
class UserChatWidget extends StatelessWidget {
  ...
  // Helper method to fix file paths
  String? _fixImagePath(String? rawPath) {
  ...
  Widget build(BuildContext context) {
    ...
    print(">>>>>>>>>>>>>>>> image ${attachments.map((item){ return item.filePath.toString();})}");
      ...
      child: Row(
        ...
        children: [
          if (!isMe) ...[
            CircleAvatar(
              backgroundImage: attachments.isNotEmpty && attachments.first.filePath != null
                  ? NetworkImage('$image_url${_fixImagePath(attachments.map((item){return item.filePath.toString();}) as String?)}')
                  : null,
              child: attachments.isEmpty || attachments.first.filePath == null
                  ? const Icon(Icons.person)
                  : null,
        ...
        child: GridView.count(
          ...
          children: List.generate(
            imageCount > maxImagesToShow ? maxImagesToShow : imageCount,
                (index) {
              final attachment = attachments[index];
              final imageUrl = _fixImagePath(attachment.filePath) != null
                  ? '$image_url${_fixImagePath(attachment.filePath)}'
                  : null;
...
class _GalleryScreenState extends State<GalleryScreen> {
  ...
  // Helper method to fix file paths (same as in UserChatWidget)
  String? _fixImagePath(String? rawPath) {
    if (rawPath == null) return null;
    return rawPath.replaceAll(r"\/", "/");
  }
  ...
  Widget build(BuildContext context) {
    ...
    print(">>>>>>>>>>>>>>> this is list of attachment ${widget.attachments.map((item){ return item.filePath.toString(); }).toList()}");
