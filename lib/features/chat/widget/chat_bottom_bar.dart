...
//     required this.initialImages, required this.isMessageSend,
//   });
//
//   @override
...
//                   ),
//                   child:      widget.isMessageSend? CircularProgressIndicator(color: Colors.white,):Icon(Icons.send_sharp, color: Colors.white),
//                 ),
//               ),
//             ],
//           ),
...
class ChatBottomBarWidget extends StatefulWidget {
  ...
  final bool isMessageSend;
  final VoidCallback onMessageSent; // নতুন callback যোগ করুন
  ...
  const ChatBottomBarWidget({
    ...
    required this.onMessageSent, // নতুন parameter
  });
...
class _ChatBottomBarWidgetState extends State<ChatBottomBarWidget> {
  ...
  @override
  void didUpdateWidget(covariant ChatBottomBarWidget oldWidget) {
    super.didUpdateWidget(oldWidget);

    // যখন parent থেকে selectedImage null আসে, তখন local stateও update করুন
    if (widget.selectedImage == null && _selectedImage != null) {
      setState(() {
        _selectedImage = null;
      });
    } else if (widget.selectedImage != _selectedImage) {
      setState(() {
        _selectedImage = widget.selectedImage;
      });
    }
          ...
          child: Row(
            children: [
              ...
              GestureDetector(
                onTap: () {
                  widget.onSendTap();
                  widget.onMessageSent(); // Send হওয়ার পর callback call করুন
                },
                child: Container(
                  height: 50,
                  width: 50,
